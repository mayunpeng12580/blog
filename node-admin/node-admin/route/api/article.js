...
//获取文章信息标题
router.get('/getArticleType/:id', (req, res) => {
    
  var  sql = 'SELECT * FROM bg_article where cate_id = ' + req.params.id ;
...
router.post('/addArticle', (req, res) => {
    ...
    var  addSql = 'INSERT INTO bg_article(cate_id,title,content,o,status,updatetime) VALUES(?,?,?,?,?,?)';
    // var  addSqlParams = req.body;
    var  addSqlParams = ['test', '1234560','180','男','2','1215664'];;
    ...
    addSqlParams[5] = moment(Date.now()).format('YYYY-MM-DD HH:mm:ss');
...
router.post("/editArticle", (req, res) => {
  ...
  var modSql = 'UPDATE bg_article SET cate_id = ? ,title = ? , content = ? ,o = ?, status = ?, updatetime = ? WHERE id = ?';
